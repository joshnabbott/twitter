...
Only tweet if the is_published? method returns true.
  <tt>twitterify :url, :title, :if => Proc.new { |record| record.is_published? }</tt>
...
Only tweet if the post is a new record being created.
  <tt>twitterify :url, :title, :if => Proc.new { |record| record.new_record? }</tt>
